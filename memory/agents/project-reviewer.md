...
- `CLAUDE.md` закоммичен как симлинк (`120000`) на отслеживаемый `AGENTS.md` — свежий clone получает актуальные правила. Если `git ls-files -s CLAUDE.md` когда-нибудь снова покажет `100644`, симлинк перезаписали обычным файлом: это регрессия, блокировать.
...
- `constants.ts` экспортирует только `CELL_WEIGHT` (`use-board-store.ts:4`) — мёртвые `BOARD_COLS`/`BOARD_ROWS`/`START_CELL_COORDS`/`TARGET_CELL_COORDS` удалены. Живой размер: `ref(50)`/`ref(28)` в `use-board-settings-store.ts:10-11`; живые старт/цель: `getDefaultCoords()` (`generate-default-board.ts:28-38`) — при 50x28 это (12,14) и (37,14). Флагать любую попытку вернуть дефолты доски в `constants.ts`.
...
- Мёртвый код (фиксировать, но НЕ удалять по своей инициативе): `src/modules/board/composables/use-eraser-mode.ts` (ноль импортёров); класс `cell` в `cell-view.vue:79` без CSS-правила; `src/modules/board/types/graph.types.ts` (реэкспортируется, но не используется); пустая директория `src/modules/board/services/`. Уже вычищено и не подлежит возврату: мёртвые константы в `constants.ts`, grid-токены в `tailwind.css`.
